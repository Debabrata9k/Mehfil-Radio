...
function Explore() {
  ...
  const toggleFavorite = (id) => {
    setFavorites((prev) => {
      let updated;
      if (prev.includes(id)) {
        updated = prev.filter((favId) => favId !== id); // remove if already favorite
      } else {
        updated = [...prev, id]; // add if not favorite
      }
      localStorage.setItem('favorites', JSON.stringify(updated)); // save to storage
      return updated;
    });
  };
        ...
        <div className='pb-20 grid gap-5 grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6'>
          {MusicData.map((item, index) => (
            ...
            >
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation(); // prevent triggering play navigation
