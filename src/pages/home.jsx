...
function Home() {
  ...
  const [favorites, setFavorites] = useState([]); // ❤️ store favorite song IDs
  ...
  // 🎶 Filter only favorite songs to show as "Top Songs"
  const topSongs = MusicData.filter((song) => favorites.includes(song.id));
        ...
        {topSongs.length > 0 ? (
          <>
            <h2 className="text-lg sm:text-xl md:text-2xl font-semibold mb-6 text-pink-400 text-center">
              ❤️ Top Songs You Love
            </h2>

            {/* Grid of Favorite Music Cards */}
              ...
              {topSongs.map((item, index) => (
                ...
                >
                  <div className="relative w-full aspect-square overflow-hidden">
                    ...
                    {/* ❤️ Heart icon to show it's a favorite */}
          ...
          <p className="text-gray-400 text-center mt-20">
            ❤️ You haven’t added any favorite songs yet. Go to{" "}
            <span className="text-purple-400 font-semibold">Explore</span> and
            tap the heart icon!
          </p>
        )}
      </div>
